...
def plt_house_x(X, y,f_wb=None, ax=None):
    ...
    ax.set_ylabel('Price (in kUSD)')
    ax.set_xlabel(f'Size (1000 m2)')
    if f_wb is not None:
...
def plt_equal_scale(X_train, X_norm, y_train):
    ...
    plt_contour_multi(X_train, y_train, w_best, b_best, ax[0], prange, 0, 1, 
                      ...
                      xlabel= "w[0] (size(m2))", ylabel="w[1] (# bedrooms)")
    #
    ...
    plt_contour_multi(X_norm, y_train, w_best, b_best, ax[1], prange, 0, 1, 
                      ...
                      xlabel= "w[0] (normalized size(m2))", ylabel="w[1] (normalized # bedrooms)")
